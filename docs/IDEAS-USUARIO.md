...
- [ ] **9781** (2026-08-03) — **El retiro de góndola al pickear tiene que ser inmediato y por el total agarrado, no recién al TP.** Mirando D15A (leg 104) y D01G (leg 122) de esta mañana: el 224 se pickeó (6 cajas) pero el módulo de Stock/Compras seguía mostrando las 6 en góndola. Causa: la baja góndola → Pickeados la escribe sólo `reconciliar_pipeline_stock()` ETAPA 1, que exige `TP` de la tanda; mientras la tanda está en curso no se escribe nada. No es por el picking parcial (la etapa mueve el real, no el esperado). Que cada `PKC` descuente ya de góndola y sume a Pickeados lo realmente agarrado, manteniendo idempotencia y single-writer (ver v5.76). — _pendiente_
