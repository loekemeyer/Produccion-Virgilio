...
>
> Última actualización: 2026-07-22 · Versión app al documentar: **v5.67**
>
> Nota: **investigación (sin bump) — 🧾 Facturar desde la app vía Isis ERP (PENDIENTE, nada aplicado)**. Charla de diseño para que el módulo **Facturación** pueda facturar sin doble carga, manteniendo la contabilidad en **Isis ERP**. Estado actual: el tilde de Facturación (`facTickNP`) es solo **marca de "ya facturé afuera"** — la factura real la hace **Isis** (emisor único hoy, vía web service a ARCA). Se descartó que la app sea **2º emisor** (punto de venta propio + certificado): Isis **solo importa comprobantes externos a mano** (Ventas > Ingresar Comprobante Manual, campo "Nro. Preimpreso"), sin API ni archivo para facturas ya autorizadas → sería recargar una por una. **Camino elegido**: la app **crea el PEDIDO en Isis por API** (`/api/ISIS_Pedido`, inserta pedido de venta) y **Isis lo factura** con su **facturación automática de pedidos** → CAE en ARCA y todo cae en Isis solo (sin 2º PDV, sin certificado, sin recarga). Datos: importe + cliente los carga el usuario en la app (no es bloqueante). **3 pendientes antes de codear** (confirmar con humano de Isis, no el bot): (1) que la **facturación automática de pedidos** corra sobre los pedidos que entran por API, y si es sola/programada o a mano; (2) **doc técnica real del endpoint** `/api/ISIS_Pedido` (URL, auth, campos); (3) **red**: Isis es **on-premise (local)** y la app corre en la nube (GitHub Pages + Supabase) → el Isis local tiene que ser **alcanzable desde internet** (IP pública segura o túnel tipo Cloudflare/VPN). Notas: la API parece estar también en la versión Escritorio/local (no solo Cloud) y sin licencia aparte, pero **confirmarlo con un humano** (las respuestas venían de un bot de Isis, riesgo de que apliquen solo a Cloud). Lo fiscal se consolida solo en ARCA bajo el mismo CUIT sin importar el PDV. Al implementar: enganchar en el módulo Facturación (al facturar → arma y postea el pedido a Isis) y guardar en la app el estado (pedido creado / facturado) para seguimiento.
>
