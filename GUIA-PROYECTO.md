...
>
> Última actualización: 2026-07-25 · Versión app al documentar: **v6.30**
>
> Nota: **v6.30 — Módulo nuevo (admin): "Líos por artículo"**. Pedido del dueño: ver **artículo por artículo cómo se arman los líos de ese ítem cuando NO se mezcla con otro**; y cuando se mezcla, contarlo aparte como **"Mezcla"**. Botón nuevo **🧷 Líos por artículo** en el panel Administración (fila secundaria, al lado de "Consultar NP / Líos"; la fila quedó en 6 columnas = 6+6). `openLiosArt()` abre un overlay (mismo patrón que los otros: header fijo, tabla con scroll interno + `thead` sticky, sin scroll de página). **Fuente:** eventos **`TAL`** (resumen de líos, `texto` p[3]) de los últimos **30/60/90 días** (selector; default 30), traídos con `supaFetchAll`. **Cálculo** (`liosArtLoad`): por cada lío parseado con `cpParseResumen`, si tiene **un solo código distinto** → es un **lío "solo/puro"** de ese artículo y suma a su distribución de tamaños `{cajas_por_lío: veces}`; si tiene **≥2 códigos distintos** → suma **+1 a "Mezcla"** en cada artículo del lío (no aporta a la distribución porque el lío no es puro). Normalización de código con `_cpNorm`. **Tabla** (`liosArtApply`, con buscador por código/nombre): `Cód · Artículo · Líos solos (N) · Cómo se arman (chips "N cj × veces", el más común resaltado) · Mezcla (N)`, ordenada por actividad (solos+mezcla) desc. Solo lectura, no toca nada, no agrega código `opcion`. Test nuevo `lios-art.cjs` (505 puro 6×3+3×1 + mezcla; 520 puro 4×2 + mezcla; filtro) en `run.sh`. Bump v6.30 / v6.30-vir.
>
> Nota: **v6.29 — Pulido de layout (Admin/Stock/racks) + ubicación por NP (AUB) visible en Carga Camión y Facturación**. Cuatro cosas pedidas por el dueño. **(1)** Panel Administración, fila **secundaria** de botones: pasó de `repeat(7,1fr)` a **`repeat(6,1fr)`** → 11 botones quedan 6+5 (antes 7+4, con la 2ª fila "colgada"). **(2) Stock** (tabla grande): `.stk-tbl th` ahora es **sticky** (`position:sticky; top:0`) y `.stk-tblwrap` pasó a `overflow:auto; max-height:calc(100dvh-240px)` → el encabezado de columnas queda fijo y la tabla scrollea **internamente** (el scroll horizontal, cuando hace falta por ser 12 columnas de números, es interno a la tabla, no de la página; en el monitor de oficina entra sin recortar). *(PPP ya se había arreglado en v6.26 —el "LOCALIDAD" partido—; sus tablas son chicas por tanda.)* **(3)** Tarjetas de **bajadas de racks** (`.stk-rk-baj`): `align-items:center`→`flex-start` → los botones ✓ Aprobar / ✗ Rechazar quedan alineados arriba con el contenido (antes desnivelados cuando el texto era más alto). **(4) Ubicación por NP (evento `AUB`, `descripcion`=ubicación, la elige el armador al TAP, v5.86) ahora se MUESTRA** — antes solo se registraba: **Carga Camión** (`ccRender`) muestra **📍 ubicación** al lado del NP (`ccFetchUbics` trae los AUB de esas NP, última carga gana) para saber dónde ir a buscar el pedido al cargar; **Facturación** (`facRender`) muestra un badge **📍** bajo el número de NP (`facFetchUbic` en el `Promise.all` de carga, `_facUbic` np→ubicación). Sin nuevo código `opcion` (AUB ya existía). De paso el ícono `📦➕` del instructivo pasó a `🧩`. Verificado headless (fila secundaria = 6 cols; 📍 aparece con ubic y no cuando no hay). Bump v6.29 / v6.29-vir.
>
