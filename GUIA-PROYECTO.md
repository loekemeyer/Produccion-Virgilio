...
>
> Última actualización: 2026-07-30 · Versión app al documentar: **v6.50**
>
> Nota: **v6.50 — Columna "FC s/Salida" por código en la solapa Stocks (entre A facturar y A guardar)**. Pedido del usuario: ver el detalle de FC s/Salida para cada código en la tabla principal de Stocks. En `stkBodyStocks` se agrega la **columna "FC s/Salida"** justo después de "A facturar" (fondo celeste `#eff6ff`), con el **chip de total** en la tira de sectores en la misma posición (entre A facturar y A guardar; el chip abre la solapa FC s/Salida). Los datos son los mismos de la solapa 🧾 FC s/Salida: se refactorizó `stkFcsLoad` extrayendo el fetch a **`stkFcsFetch()`** (con guard `fcsLoading` y captura de `_stk` para tolerar que cierren el overlay a mitad de carga), y la solapa Stocks lo dispara **en segundo plano** la primera vez (muestra "…" mientras carga y se re-renderiza sola). Click en un número > 0 → **`stkOpenFcsArt(cod)`**: abre la solapa FC s/Salida ya **filtrada por ese código** (detalle por NP). Las filas con FC s/salida > 0 no se ocultan aunque el artículo esté en 0 en todos los depósitos. ⚠ La columna/chip muestran SIEMPRE el estado **actual** (no es un depósito de `Movimientos_Stock`): no respetan el corte de conteo ni "Ver stock a esa fecha/hora" (tooltip lo aclara). `colspan` de la tabla +1. Smoke: se agregan `stkFcsLoad`/`stkFcsFetch`/`stkOpenFcsArt`/`stkBodyFcs` a la lista de funciones requeridas. Bump `APP_VERSION` + `SW_VERSION` `v6.50`.
>
