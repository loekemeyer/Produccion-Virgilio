...
>
> Última actualización: 2026-07-16 · Versión app al documentar: **v5.40**
>
> Nota: **v5.40 — Módulo "📦 Importados / OC" (réplica del Excel QUIEBRE, con stock ROTATIVO)**. Botón nuevo en el panel de supervisor (primarios). Réplica del Excel *"QUIEBRE de art importados"* (motor = hoja **"Todos"**) para generar las OCs de los **artículos importados**, en **UNIDADES**. **Fórmula (por artículo)**: `Máximo = Meses_objetivo × Est Madre` · `Total Stock = Stock Actual (rotativo) + Pedido en Curso` · `Genera Pedido = máx(0, Máximo − Total Stock)` · `Meses cobertura = Total Stock ÷ Est Madre` (semáforo: <1,5 quiebre) · `Total FOB = FOB_uni × (Pedido Manual si se cargó, si no Genera Pedido)`. **Est Madre** (el corazón): efectiva = **override manual** > **live** (`proyeccion_madre` = algoritmo Estadística Madre de PaginaLK, solo LK/Loke) > **seed** del Excel (CH usa seed, no hay proyección de Chef todavía). El algoritmo PaginaLK está decodificado: por ítem, suma una proyección **limpia por cliente** = promedio mensual de una ventana de 24 meses **descontando los meses disruptivos** (picos de una vez que no son recurrentes [otro mes ≥0,8×] ni progresivos [mes previo ≥0,5×], umbral 1,5×promedio); denominador = meses activos. **Multi-proveedor**: FOB por `(código, proveedor)`; un mismo código con >1 proveedor = filas **alternas** (`principal=false`, marcadas *alt*, no generan OC ni suman al total, son cotización comparativa). **Stock ROTATIVO event-sourced** (`Stock Inicial − Ventas + Compras`): las **ventas se derivan solas** del armado real (`Entregas_Virgilio` para LK/Loke, `Entregas Tallerista Cervantes` para CH, cajas→unidades × `uni_x_caja`) desde la fecha del `inicial`; **compras/ajustes** se cargan a mano (botón ± stk). Perilla global **Meses objetivo** (=8, editable). Botón **Generar OC** → escribe `Ordenes_Compra` (rubro `Importado`, unidad `Unidades`, por proveedor). **Tablas nuevas (Supabase, aditivas)**: `Importados` (161 filas: universo del Excel, `cod_art, marca[LK/Loke/CH], proveedor, descripcion, fob_uni, uni_x_caja, est_madre_seed, est_madre_override, pedido_manual, pedido_curso, principal`), `Importados_Mov_Stock` (event-sourced, `inicial/compra/ajuste`), `Importados_Config` (fila única `meses_objetivo`), y la vista **`v_importados_ordenes`** (motor calculado, SECURITY DEFINER a propósito — necesita leer `Entregas Tallerista Cervantes` que la anon no puede). RLS: read anon+authenticated, write authenticated (sesión de supervisor Google, `facAuthWriteHeaders`), movimientos insert solo authenticated. Front: `openImportadosAdmin`/`impRender`/`impCalc`/`impBody`/`impGuardar`/`impMov`/`impGenerarOC` (CSS `imp-*`). Seed inicial de stock = Stock Actual del Excel (por `(cod,marca)`, con continuidad). ⚠ 42/161 sin `uni_x_caja` (no estaban en `OC_Maximos`/`proyeccion_madre`) → su rotativo no auto-descuenta ventas hasta setear el `uni_x_caja` (editable). ⚠ La Est Madre del Excel (pegado viejo) diverge del live (ej. 951E Excel 420 vs live 228) — el módulo usa live para LK y lo muestra con un punto de color (verde=live, ámbar=seed, azul=override). Excel decodificado: `I3`=Meses objetivo, `N`=Est Madre (pegada), `K`=Stock Actual (VLOOKUP a hojas Stock Loeke/Chef, REEMPLAZADO por el rotativo).
>
